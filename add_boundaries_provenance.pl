...
# Called by steady-state-boundaries.c to add information to the provenance file
...
print $out_fh "steady-state-boundaries.c ran a simulation at " . localtime . " with K=$K and $N_time_steps time steps, using code from commit $Repo::hash";
close $out_fh;
